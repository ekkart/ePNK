...
public class SAXPNMLHandler extends SAXXMLHandler implements IAttributeProvider {
	...
	*/
	...
	private Vector<AssocClass> assocObjects = new Vector<AssocClass>();
	// EK end
	...
	protected EStructuralFeature getFeature(EObject object, String prefix,
		...
		*/
		...
		if (object instanceof AssocClass && isElement) {
			...
			if (extendedPNMLMetaData.getElementFactory(name) != null) {
	...
	protected void createObject(EObject peekObject, EStructuralFeature feature) {
		...
		*/
		...
		if (this.currentAssocObject != null) {
			IPNMLFactory factory = extendedPNMLMetaData.getElementFactory(this.currentElement);
			if (factory != null) {
				currentObject = factory.createObject(null, this.currentAssocObject, this.currentElement, null, null);
				currentElement = null;
	...
	protected void handleObjectAttribs(EObject obj) {
		...
		if (featureList != null && featureList.size() > 0) {
			for (EStructuralFeature feature: featureList) {
				if (feature != PnmlcoremodelPackage.eINSTANCE.getPetriNet_Type()) {
					// eki: This feature does not need to be set anymore; it cannot
					//      be removed from the mapping, since this is needed for
					//      serialising the Petri net propery
	...
	public void endDocument() {
        // TODO: This is a terrible hack to remove remaining AssocClasses that were not
		//       removed by setting the respective elements (this happens if the respective
		//       associations remain empty in the XML file); this needs solved in a better
		//       way.
		for (EObject object: this.assocObjects ) {
	...
	protected void setFeatureValue(EObject object, EStructuralFeature feature,
		...
		if (value instanceof AssocClassImpl) {
			// May 7, 2013, ekki: Do not add the element representing an association (AssocClass)
			//                    to the model at all.
			return;
		}
	...
	/* (non-Javadoc)
	 * @see org.eclipse.emf.ecore.xmi.impl.XMLHandler#setFeatureValue(org.eclipse.emf.ecore.EObject, org.eclipse.emf.ecore.EStructuralFeature, java.lang.Object)
	 */
	@Override
	protected void setFeatureValue(EObject object, EStructuralFeature feature,
			Object value) {
