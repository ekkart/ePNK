...
public class PNMLSave extends XMLSaveImpl {
	...
	protected boolean shouldSaveFeature(EObject o, EStructuralFeature f) {
		...
		// TODO eki: Make sure that empty bundle features when the bundle feature is empty
		//           we would probably need to make that configurable since we probably do 
		//           not want to do that for all empty bundle features.
		//           (reason was that the empty parameter feature must be there.
		if (this.extendedPNMLMetaData.getBundleFeatureElement(f) != null) {
			return true;
		}
	...
	protected void saveContainedMany(EObject o, EStructuralFeature f) {
		...
		if (name != null) {
			...
			*/
			...
			// Fixed the problem with loading empty lists (with a hack).
			// Since the standard seems to require the empty element anyway,
			// I write it now, even if it is empty (I would change that
			// in the standard eventually.
